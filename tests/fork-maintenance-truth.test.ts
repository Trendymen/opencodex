...
describe("Fork maintenance truth", () => {
  ...
  test("grounds every active official comparison in v2.37.0 evidence", () => {
    const evidence = {
      "火山方舟 Agent Plan GLM/Kimi 与智谱 GLM Responses 兼容": [
        "`v2.37.0:src/adapters/openai-responses.ts`（blob `047c60a6a3fafefaa5d4ea0fea199565286d5054`，v2.37 未变更）",
      ...
      "Standalone web search 能力注入": [
        "`v2.37.0:src/codex/inject.ts`（blob `72be57878470077e9b3c434726aea329e007d79c`）",
      ...
      "智谱 BigModel Codex 模型发现": [
        "`v2.37.0:src/providers/model-discovery.ts`（blob `ada0bd2aecc196e003d0b1720c96d864e4793dbc`）",
        "`src/providers/model-discovery.ts`（blob `85ea01d624b128d56400f4b699b95b32517de639`）",
        "`c9446e0b5cddb90a0569d8e59913a91ae7eaa893`",
      ],
      "默认测试 runner 与负载敏感隔离": [
        "`v2.37.0:tests/update-stop-first.test.ts`（blob `0f7fd7ff55ec23cbdea4d157df61262bd9f8cd8e`，merge `fe063d16ef620a148ab425cfffe63a8936d00e52`）",
    ...
    for (const [title, anchors] of Object.entries(evidence)) {
      ...
      expect(active).toContain("v2.37.0");
  ...
  test("separates Fork strict backend recovery from official v2.35 turn termination", () => {
    ...
    expect(compactWhitespace(recovery)).toContain(
      "`v2.37.0:src/server/responses/agent-task-recovery.ts`（blob `8b409e175bfb83345ac147ccbeb4b5bc4d462fcf`，相对 `v2.34.0` 新增官方 cache admission 重构）",
    );
