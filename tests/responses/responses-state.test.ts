...
async function runShutdownBudgetChild(
  ...
  const child = Bun.spawn([
    process.execPath,
    helperPath("responses-state-shutdown-budget-child.ts"),
    scenario,
  ], {
    cwd: repoRoot(),
    env: { ...process.env },
...
async function runNeverSettlingAclChild(
  ...
  const timeoutMs = Math.max(watchdogMs(1_500), isolationBudgetMs(1_500));
  const child = Bun.spawn([
    process.execPath,
...
describe("Responses previous_response_id state", () => {
  ...
  }, { timeout: (2 * Math.max(watchdogMs(1_500), isolationBudgetMs(1_500))) + 2_000 });
