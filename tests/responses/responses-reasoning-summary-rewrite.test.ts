...
function projectedRawSummary(item: Record<string, unknown>): string {
  const summary = item.summary as Array<{ text: string }>;
  return summary.map((part, index) => index === 0
    ? part.text.slice(part.text.indexOf("\n\n") + 2)
    : part.text).join("");
}

function projectedRawDeltas(deltas: string[]): string {
  return deltas.map((delta, index) => index === 0
    ? delta.slice(delta.indexOf("\n\n") + 2)
    : delta).join("");
}
...
describe("responses reasoning summary channel rewrite", () => {
  ...
  test("streams a long contiguous sequence before its terminal event without losing summary text", () => {
    const budget = createTestTranslatorBudget();
    const stream = createReasoningSummaryChannelBlockRewrite({ translatorBudget: budget });
    const chunks = Array.from({ length: 10_000 }, (_, index) => index === 0 ? "First sentence." : "x");
    const raw = chunks.join("");
    const summaryDeltas: string[] = [];
    const collectSummaryDeltas = (payloads: Record<string, unknown>[]): void => {
      for (const payload of payloads) {
        if (payload.type === "response.reasoning_summary_text.delta" && typeof payload.delta === "string") {
          summaryDeltas.push(payload.delta);
        }
      }
    };

    for (const [sequenceNumber, delta] of chunks.entries()) {
      const output = ssePayloads(stream(sseBlock({
        type: "response.reasoning_text.delta",
        item_id: "rs_contiguous",
        output_index: 0,
        sequence_number: sequenceNumber,
        delta,
      })));
      collectSummaryDeltas(output);
    }

    expect(summaryDeltas.length).toBeGreaterThan(1);
    expect(projectedRawDeltas(summaryDeltas).length).toBeGreaterThan(raw.length - 500);
    expect(budget.snapshot().currentBytes).toBe(10_000 * 32);
    const terminal = ssePayloads(stream(sseBlock({
      type: "response.output_item.done",
      output_index: 0,
      item: {
    ...
    collectSummaryDeltas(terminal);
    const item = terminal.at(-1)?.item as Record<string, unknown>;
    expect(item.content).toEqual([{ type: "reasoning_text", text: raw }]);
    expect(projectedRawDeltas(summaryDeltas)).toBe(raw);
    expect(projectedRawSummary(item)).toBe(raw);
    expect(budget.snapshot().currentBytes).toBe(0);
  });

  test("deduplicates nonempty deltas while preserving distinct out-of-order sequence numbers", () => {
    const budget = createTestTranslatorBudget();
    const stream = createReasoningSummaryChannelBlockRewrite({ translatorBudget: budget });
    const delta = (sequenceNumber: number, text: string) => stream(sseBlock({
      type: "response.reasoning_text.delta",
      item_id: "rs_bridge",
      output_index: 0,
      sequence_number: sequenceNumber,
    ...
    delta(1, "One.");
    delta(3, "Three.");
    expect(budget.snapshot().currentBytes).toBe(64);
    expect(delta(1, "Duplicate one.")).toEqual([]);
    expect(budget.snapshot().currentBytes).toBe(64);
    delta(2, "Two.");
    expect(budget.snapshot().currentBytes).toBe(96);
    expect(delta(3, "Duplicate three.")).toEqual([]);
    expect(budget.snapshot().currentBytes).toBe(96);
    ...
    const terminal = ssePayloads(stream(sseBlock({
      ...
      item: {
        ...
        id: "rs_bridge",
        status: "completed",
        content: [{ type: "reasoning_text", text: "One.Three.Two." }],
        summary: [],
      },
    })));
    const item = terminal.at(-1)?.item as Record<string, unknown>;
    expect(projectedRawSummary(item)).toBe("One.Three.Two.");
    expect(projectedRawSummary(item)).not.toContain("Duplicate");
    expect(budget.snapshot().currentBytes).toBe(0);

    stream(sseBlock({
      type: "response.reasoning_text.delta",
      item_id: "rs_join_following",
      output_index: 1,
      sequence_number: 2,
      delta: "First.",
    }));
    expect(budget.snapshot().currentBytes).toBe(32);
    stream(sseBlock({
      type: "response.reasoning_text.delta",
      item_id: "rs_join_following",
      output_index: 1,
    ...
    expect(budget.snapshot().currentBytes).toBe(64);
    const leadingTerminal = ssePayloads(stream(sseBlock({
      type: "response.output_item.done",
      output_index: 1,
      item: {
        type: "reasoning",
        id: "rs_join_following",
        status: "completed",
        content: [{ type: "reasoning_text", text: "First.Before." }],
        summary: [],
      },
    })));
    expect(projectedRawSummary(leadingTerminal.at(-1)?.item as Record<string, unknown>)).toBe("First.Before.");
  });

  test("accepts a sparse sequence stream beyond the former per-item and Fork quota caps", () => {
    const budget = createTestTranslatorBudget();
    const stream = createReasoningSummaryChannelBlockRewrite({ translatorBudget: budget });
    for (let sequenceNumber = 0; sequenceNumber < 20_000; sequenceNumber += 2) {
      stream(sseBlock({
        type: "response.reasoning_text.delta",
    ...
    stream(sseBlock({
      type: "response.output_item.done",
      output_index: 0,
      item: { type: "reasoning", id: "rs_sequence_cap", status: "completed", content: [], summary: [] },
    }));
    expect(budget.snapshot().currentBytes).toBe(0);
  });

  test("shares the supplied translator budget and does not retain a rejected sequence number", () => {
    const budget = createTestTranslatorBudget({ maxTurnBytes: 96 });
    ...
    clientRewrite(sseBlock({ type: "response.reasoning_text.delta", item_id: "client", output_index: 0, sequence_number: 0, delta: "" }));
    replayRewrite(sseBlock({ type: "response.reasoning_text.delta", item_id: "replay", output_index: 1, sequence_number: 0, delta: "" }));
    clientRewrite(sseBlock({ type: "response.reasoning_text.delta", item_id: "client", output_index: 0, sequence_number: 1, delta: "" }));
    expect(budget.snapshot().currentBytes).toBe(96);
    let overflow: unknown;
    try {
      replayRewrite(sseBlock({
        type: "response.reasoning_text.delta",
        item_id: "replay",
        output_index: 1,
        sequence_number: 1,
    ...
    expect(overflow.limitBytes).toBe(96);

    clientRewrite(sseBlock({
      type: "response.output_item.done",
      output_index: 0,
      item: { type: "reasoning", id: "client", status: "completed", content: [], summary: [] },
    }));
    expect(budget.snapshot().currentBytes).toBe(32);
    replayRewrite(sseBlock({ type: "response.reasoning_text.delta", item_id: "replay", output_index: 1, sequence_number: 1, delta: "" }));
    expect(budget.snapshot().currentBytes).toBe(64);
    replayRewrite.dispose!();
    expect(budget.snapshot().currentBytes).toBe(0);
  });

  test("owns and disposes a default translator budget on every stream terminal path", () => {
    ...
    const terminals: Array<readonly [string, (stream: ReturnType<typeof createReasoningSummaryChannelBlockRewrite>) => void]> = [
      ["output_item.done", stream => { stream(sseBlock({
        type: "response.output_item.done",
        output_index: 0,
        item: { type: "reasoning", id: "rs_default", status: "completed", content: [], summary: [] },
      })); }],
    ...
    for (const [name, terminate] of terminals) {
      ...
      expect(translatorLiveBudgetCountForTests(), name).toBe(before + 1);
      terminate(stream);
      expect(translatorLiveBudgetCountForTests(), name).toBe(before);
      expect(translatorAggregateCurrentBytesForTests(), name).toBe(0);
    }
  });
