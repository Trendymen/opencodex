...
type ResolveLocalInstallRuntime = (
  packageRoot: string,
  name: string,
  version: string,
  run: (command: string[]) => CommandResult,
  options?: { platform?: NodeJS.Platform; findExecutable?: (command: string) => string | null },
) => { command: [string, string]; launcherPath: string };
type ResolveCurrentLocalInstallRuntime = (
  packageRoot: string,
  name: string,
  run: (command: string[]) => CommandResult,
) => { runtime: { command: [string, string]; launcherPath: string }; version: string };
...
async function runtimeResolver(): Promise<ResolveLocalInstallRuntime> {
  const module = await import("../../scripts/install-local");
  const resolveLocalInstallRuntime = (module as unknown as {
    resolveLocalInstallRuntime?: ResolveLocalInstallRuntime;
  }).resolveLocalInstallRuntime;
  expect(typeof resolveLocalInstallRuntime).toBe("function");
  if (!resolveLocalInstallRuntime) throw new Error("resolveLocalInstallRuntime is unavailable");
  return resolveLocalInstallRuntime;
}

async function currentRuntimeResolver(): Promise<ResolveCurrentLocalInstallRuntime> {
  const module = await import("../../scripts/install-local");
  const resolveCurrentLocalInstallRuntime = (module as unknown as {
    resolveCurrentLocalInstallRuntime?: ResolveCurrentLocalInstallRuntime;
  }).resolveCurrentLocalInstallRuntime;
  expect(typeof resolveCurrentLocalInstallRuntime).toBe("function");
  if (!resolveCurrentLocalInstallRuntime) throw new Error("resolveCurrentLocalInstallRuntime is unavailable");
  return resolveCurrentLocalInstallRuntime;
}
...
describe("install-local resolves the package selected by Volta", () => {
  ...
  test("binds service repair to the target Volta package launcher and its installed CLI", async () => {
    const resolveLocalInstallRuntime = await runtimeResolver();
    const image = writeVoltaImage(name, version);
    const cliDirectory = join(image.packageRoot, "src", "cli");
    mkdirSync(cliDirectory, { recursive: true });
    writeFileSync(join(cliDirectory, "index.ts"), "export {};\n", "utf8");

    expect(resolveLocalInstallRuntime(image.packageRoot, name, version, command => {
      expect(command).toEqual(["volta", "which", "ocx"]);
      return { status: 0, stdout: image.wrapper, stderr: "" };
    })).toEqual({
      command: [process.execPath, realpathSync(join(image.packageRoot, "src", "cli", "index.ts"))],
      launcherPath: image.wrapper,
    });
  });

  test("rebinds rollback recovery to the restored version in the transaction's stable package root", async () => {
    const resolveCurrentLocalInstallRuntime = await currentRuntimeResolver();
    const image = writeVoltaImage(name, version);
    const cliDirectory = join(image.packageRoot, "src", "cli");
    mkdirSync(cliDirectory, { recursive: true });
    writeFileSync(join(cliDirectory, "index.ts"), "export {};\n", "utf8");
    const restoredVersion = "2.36.0-ben.2";
    writeFileSync(join(image.packageRoot, "package.json"), JSON.stringify({
      name,
      version: restoredVersion,
      bin: { ocx: "./bin/ocx.mjs" },
    }), "utf8");

    expect(resolveCurrentLocalInstallRuntime(image.packageRoot, name, command => {
      expect(command).toEqual(["volta", "which", "ocx"]);
      return { status: 0, stdout: image.wrapper, stderr: "" };
    })).toEqual({
      runtime: {
  ...
  test.skipIf(process.platform === "win32")("rejects a launcher the service selector would skip for another installation", async () => {
    const resolveLocalInstallRuntime = await runtimeResolver();
    const { stableLauncherEntry } = await import("../../src/service");
    const image = writeVoltaImage(name, version);
    const other = writeVoltaImage(name, version);
    mkdirSync(join(image.packageRoot, "src", "cli"), { recursive: true });
    writeFileSync(join(image.packageRoot, "src", "cli", "index.ts"), "export {};\n");
    const env = { PATH: [dirname(image.wrapper), dirname(other.wrapper)].join(delimiter) };
    expect(stableLauncherEntry({ env })).toBe(image.wrapper);
    chmodSync(image.wrapper, 0o644);
    expect(stableLauncherEntry({ env })).toBe(other.wrapper);
    expect(() => resolveLocalInstallRuntime(image.packageRoot, name, version, () => ({
      status: 0, stdout: image.wrapper, stderr: "",
    }))).toThrow("could not bind local install runtime");
  });

  test("binds the runtime through a Windows npm shim and rejects a shim for a different package", async () => {
    const resolveLocalInstallRuntime = await runtimeResolver();
    const prefix = temporaryRoot("windows-runtime");
    const npmRoot = join(prefix, "node_modules");
    const installed = writeNpmGlobalPackage(npmRoot, name, version);
    mkdirSync(join(installed.root, "src", "cli"), { recursive: true });
    const cli = join(installed.root, "src", "cli", "index.ts");
    writeFileSync(cli, "export {};\n");
    const shim = writeWindowsNpmShim(prefix, installed.bin);
    const run = (command: string[]): CommandResult => command[0] === "volta"
      ? { status: -1, stdout: "", stderr: "", errorCode: "ENOENT" }
      : { status: 0, stdout: npmRoot, stderr: "" };
    const options = { platform: "win32" as const, findExecutable: () => shim };
    expect(resolveLocalInstallRuntime(installed.root, name, version, run, options)).toEqual({
      command: [process.execPath, realpathSync(cli)], launcherPath: shim,
    });
    const other = writeNpmGlobalPackage(npmRoot, "other-package", version);
    writeWindowsNpmShim(prefix, other.bin);
    expect(() => resolveLocalInstallRuntime(installed.root, name, version, run, options))
      .toThrow("could not bind local install runtime");
  });

  test.skipIf(process.platform === "win32")("binds the runtime through the npm executable symlink", async () => {
    const resolveLocalInstallRuntime = await runtimeResolver();
    const prefix = temporaryRoot("unix-runtime");
    ...
    const launcher = join(prefix, "bin", "ocx");
    symlinkSync(installed.bin, launcher);
    const runtime = resolveLocalInstallRuntime(installed.root, name, version, command => command[0] === "volta"
      ? { status: -1, stdout: "", stderr: "", errorCode: "ENOENT" }
      : { status: 0, stdout: npmRoot, stderr: "" }, { findExecutable: () => launcher });
    expect(runtime).toEqual({ command: [process.execPath, realpathSync(cli)], launcherPath: launcher });
  });
