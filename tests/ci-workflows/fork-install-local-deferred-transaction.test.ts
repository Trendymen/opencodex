...
import { createVoltaRegistrationSync } from "../../scripts/install-local-volta";

async function registrationWrapper() {
  const installer = await import("../../scripts/install-local");
  const wrap = (installer as unknown as {
    withLocalInstallRegistration?: (
      transaction: { commit(): { ok: boolean; phase: string }; rollback(): { ok: boolean; phase: string } },
      registration: {
        sync(): void; verify(): void;
        rollback?<T extends { ok: boolean; phase: string }>(restorePackage: () => T): T;
      } | null,
    ) => { commit(): { ok: boolean; phase: string }; rollback(): { ok: boolean; phase: string } };
  }).withLocalInstallRegistration;
  expect(typeof wrap).toBe("function");
  if (!wrap) throw new Error("registration wrapper is unavailable");
  return wrap;
}
...
function deferredFixture(withVolta = false) {
  const root = mkdtempSync(join(tmpdir(), "ocx-deferred-transaction-"));
  roots.push(root);
  const voltaHome = join(root, "volta");
  const packageDir = withVolta
    ? join(voltaHome, "tools", "image", "packages", "@bitkyc08", "opencodex",
      ...(process.platform === "win32" ? [] : ["lib"]), "node_modules", "@bitkyc08", "opencodex")
    : join(root, "scope", "opencodex");
  const stagedTree = join(root, "prepared");
  packageTree(packageDir, "2.40.0-ben.2");
  packageTree(stagedTree, "2.40.0-ben.3");
  const registrationFiles = [
    join(voltaHome, "tools", "user", "packages", "@bitkyc08", "opencodex.json"),
    join(voltaHome, "tools", "user", "bins", "ocx.json"),
  ];
  if (withVolta) {
    const shared = { version: "2.40.0-ben.2", platform: { node: "22.21.1", npm: null, pnpm: null, yarn: null }, manager: "Npm" };
    const configs = [{ ...shared, name: pkgName, bins: ["ocx"] }, { ...shared, name: "ocx", package: pkgName }];
    registrationFiles.forEach((path, index) => {
      mkdirSync(dirname(path), { recursive: true });
      writeFileSync(path, JSON.stringify(configs[index]));
    });
  }
  const registration = withVolta ? createVoltaRegistrationSync(packageDir, pkgName, { env: { VOLTA_HOME: voltaHome } }) : null;
  registration?.sync();
  ...
  return { root, packageDir, result, registration, registrationFiles };
}
...
describe("install:local deferred package transaction", () => {
  test("restores the old package when the new bin set is rejected by Volta registration", async () => {
    const wrap = await registrationWrapper();
    const { packageDir, result, registration, registrationFiles } = deferredFixture(true);
    const manifestPath = join(packageDir, "package.json");
    const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
    manifest.bin.extra = "bin/ocx.mjs";
    writeFileSync(manifestPath, JSON.stringify(manifest));
    await expect(runLocalInstallLifecycle(true, {
      stop: () => {}, verifyStopped: () => {},
      replace: () => wrap({ commit: result.commit!, rollback: result.rollback! }, registration),
      afterReplace: () => { registration!.sync(); },
      restart: () => { registration!.verify(); }, ready: () => {},
    })).rejects.toThrow("Volta package registration does not match its managed package");
    expect(version(packageDir)).toBe("2.40.0-ben.2");
    expect(registrationFiles.map(path => JSON.parse(readFileSync(path, "utf8")).version))
      .toEqual(["2.40.0-ben.2", "2.40.0-ben.2"]);
  });

  test("enters registration recovery protection before restoring package bytes", async () => {
    const wrap = await registrationWrapper();
    const events: string[] = [];
    const wrapped = wrap({
      commit: () => ({ ok: true, phase: "committed" }),
      rollback: () => { events.push("restore-package"); return { ok: true, phase: "rolled-back" }; },
    }, {
      sync: () => { events.push("unguarded-sync"); }, verify: () => {},
      rollback: restore => {
        events.push("open-registration-journal");
        const result = restore();
        events.push("sync-and-close-registration-journal");
        return result;
      },
    });
    expect(wrapped.rollback().ok).toBe(true);
    expect(events).toEqual(["open-registration-journal", "restore-package", "sync-and-close-registration-journal"]);
  });

  test("keeps real Volta records and package bytes aligned through readiness failure and rollback", async () => {
    const wrap = await registrationWrapper();
    const { packageDir, result, registration, registrationFiles } = deferredFixture(true);
    ...
    await expect(runLocalInstallLifecycle(true, {
      ...
      replace: () => wrap({ commit: result.commit!, rollback: result.rollback! }, registration),
      afterReplace: () => { registration!.sync(); },
      restart: () => { registration!.verify(); },
      ready: () => {
        if (!failed) {
          failed = true;
          expect(registrationFiles.map(path => JSON.parse(readFileSync(path, "utf8")).version))
            .toEqual(["2.40.0-ben.3", "2.40.0-ben.3"]);
          throw new Error("synthetic new-service failure");
        }
        registration!.verify();
      },
    })).rejects.toThrow("synthetic new-service failure");
    expect(version(packageDir)).toBe("2.40.0-ben.2");
    expect(registrationFiles.map(path => JSON.parse(readFileSync(path, "utf8")).version))
      .toEqual(["2.40.0-ben.2", "2.40.0-ben.2"]);
  });

  test("refuses to commit package bytes when registration validation fails", async () => {
    const wrap = await registrationWrapper();
    const { packageDir, result } = deferredFixture();
    const wrapped = wrap({ commit: result.commit!, rollback: result.rollback! }, {
      sync: () => {},
      verify: () => { throw new Error("registration drift"); },
    });
    expect(wrapped.commit()).toMatchObject({ ok: false, phase: "registration-verify" });
  ...
  for (const restart of [true, false]) {
    test(`restores registration after package rollback with restart=${restart}`, async () => {
      const wrap = await registrationWrapper();
      const { packageDir, result } = deferredFixture();
      const versions: string[] = [];
      const registration = {
        sync: () => { versions.push(version(packageDir)); },
        verify: () => { expect(versions.at(-1)).toBe(version(packageDir)); },
        rollback: <T extends { ok: boolean; phase: string }>(restore: () => T): T => {
          const result = restore();
          if (result.ok) { registration.sync(); registration.verify(); }
          return result;
        },
      };
      const wrapped = wrap({ commit: result.commit!, rollback: result.rollback! }, registration);
      let injected = false;
      await expect(runLocalInstallLifecycle(restart, {
        stop: () => {}, verifyStopped: () => {}, replace: () => wrapped,
        afterReplace: () => {
          registration.sync();
          if (!restart) throw new Error("post-install configuration failed");
        },
        restart: () => { registration.verify(); },
        ready: () => {
          if (!injected) { injected = true; throw new Error("new service not ready"); }
          registration.verify();
        },
      })).rejects.toThrow(restart ? "new service not ready" : "post-install configuration failed");
      expect(versions).toEqual(["2.40.0-ben.3", "2.40.0-ben.2"]);
      expect(version(packageDir)).toBe("2.40.0-ben.2");
    });
  }

  test("leaves ordinary npm package transactions unchanged", async () => {
    const wrap = await registrationWrapper();
    const transaction = { commit: () => ({ ok: true, phase: "committed" }), rollback: () => ({ ok: true, phase: "rolled-back" }) };
    expect(wrap(transaction, null)).toBe(transaction);
  });
