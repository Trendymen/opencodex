...
| Provider quotas and tests | `src/server/management/provider-routes.ts` — `GET /api/provider-quotas`, `POST /api/providers/test`, `GET /api/provider-presets`. A quota read may be served from cache or force-refreshed; absent quota data is reported as unknown rather than as a measured zero. |
| Provider context caps | `src/server/management/provider-context-cap-routes.ts` — `GET/PUT /api/provider-context-caps`. A failed save leaves the live global value, active caps, and remembered values unchanged. |
