...
| DeepSeek first-party | `src/providers/registry.ts` | `deepseek-flash` is the current V4.1 id and `deepseek-v4-flash` is its compatibility alias. A 2026-09-10 live probe verified the legacy id with image input on Chat Completions and Responses; both spellings therefore advertise `text` + `image`, while `deepseek-chat` and `deepseek-reasoner` remain vision-sidecar consumers. The canonical spelling has not been probed separately after the rename. |
