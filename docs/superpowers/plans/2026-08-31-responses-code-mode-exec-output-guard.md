...
- Modify exactly one production file: `src/adapters/openai-responses.ts`.
- Add exactly one isolated test file: `tests/responses-code-mode-exec-output-guard.test.ts`.
- Do not modify `src/adapters/tool-catalog-nudge.ts`, `src/adapters/exec-tool-result-normalize.ts`, provider registry/config, Responses core, relay, routing, GUI, docs-site, or structure documents.
...
**Files:**
- Add: `docs/superpowers/specs/2026-08-31-responses-code-mode-exec-output-guard-design.md`
- Add: `docs/superpowers/plans/2026-08-31-responses-code-mode-exec-output-guard.md`
- Preserve: all other tracked and untracked files

**Interfaces:**
- Consumes: approved `SPEC_DOCUMENT` and `PLAN_DOCUMENT` decisions.
- Produces: one documentation commit and a recorded predecessor/official baseline for the implementation review package.
...
Expected before the documentation commit:
...
`4fbafab9f` must still own only `src/adapters/openai-responses.ts` plus `tests/responses-call-idless-tool-output.test.ts`; `c838e5e88` must own only the Volta installer source/test. If any unrelated dirty file is present, do not stage it.
...
These are observations, not immutable expectations. If refs move, record the new values and verify that the predecessor helper still appears immediately after generic empty-output annotation and immediately before broad orphan repair. The task remains valid only if the new change can still stay in one production file.
...
```bash
git commit -m "docs: 规划 Responses code-mode 空输出防护"
```

Record the commit id as `DOC_COMMIT` for the final review package.

- [ ] **Step 5: Confirm implementation starts from a clean tree**
...
Expected: empty output. If another task writes after Step 1, stop and re-resolve ownership before creating the test file.
...
### Task 2: Implement the Shared Eligibility, Paired Diagnosis, and Preventive Echo GREEN
...
- Produces: private `isRoutedCodeModeExecOutputGuardEligible()`, `normalizePairedCodeModeExecOutputs()`, and `appendCodeModeResultEchoSentence()`.
...
- [ ] **Step 2: Add the one shared request-level eligibility helper**
...
- [ ] **Step 3: Add the copy-on-change paired custom-exec normalizer**
...
- [ ] **Step 4: Add the idempotent instruction appender**
...
- [ ] **Step 5: Activate the paired normalizer at the predecessor-owned early boundary**
...
- [ ] **Step 6: Activate prevention immediately before the existing routed progress contract**
...
- [ ] **Step 8: Run directly coupled regressions**
...
- [ ] **Step 9: Check the task-scoped diff and minimum official delta**
...
Expected: no whitespace errors; intent-to-add makes the working diff name one production file plus the complete new test. The single-ended merge-base comparison includes both committed Fork history and the current uncommitted adapter edits, so its post-implementation numstat must grow only by imports, three private helpers, and two guarded calls. If a shared helper or third production file appears necessary, stop and revise the Spec instead of widening silently.
...
Expected: all commands exit 0. Do not replace `test:changed` with bare `bun test`. Run the full `bun run test` only if `test:changed` reports an ambiguous indirect dependency or the user explicitly requests the PR-ready full gate.
...
- focused GREEN, coupled regression, typecheck, `test:changed`, privacy, and diff-check outputs;
- named risks: semantic code-mode detection, shared eligibility, canonical isolation, compaction isolation, same-kind unique pairing, duplicate IDs, generic annotation precedence, predecessor call-ID-less repair, broad orphan policy, input immutability, and minimum official delta.
...
Expected staged names are exactly the production adapter and new isolated test. The document commit is already separate, and no installer, relay, core, existing test, config, or generated file may be staged.
...
- [ ] Production scope is exactly one adapter file; test scope is exactly one new isolated test file.
- [ ] Focused tests, coupled regressions, typecheck, `test:changed`, privacy scan, and diff check pass.
