...
1. fetch 后固定本地/远端 main、upstream-release、sync/vX.Y.Z 和目标 Fork Tag 的 raw/peeled SHA。本地 main 与 marker 与远端一致；漂移则停止。
...
12. 一次 git push --atomic：末尾文档 commit 到 main 和 sync/vX.Y.Z，官方 Tag SHA 到 upstream-release，Fork Tag 与官方基线 Tag refspec 不加 +。三个 branch 用显式 expected-SHA --force-with-lease。任一漂移、冲突、失败 fail closed；禁止普通 force 和拆分推送。
