...
export function createInboundResponsesDebugObserver(
  ...
  const textSampleLimit = isProviderTextDebugEnabled()
    ? Math.min(
...
export function persistInboundResponsesDebugSummary(args: {
  ...
  const textCaptureEnabled = isProviderTextDebugEnabled();
  const textSamples = textCaptureEnabled ? observerSummary.textSamples ?? [] : [];
