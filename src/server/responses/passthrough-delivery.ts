   ...
   if (usesCodexForwardPoolAuth(admissionState.authCtx, route.provider)) {
    ...
    if (isEventStream && upstreamResponse.body) {
      ...
      const rawInboundSseInspector = !webSearchBridgePlan && terminalRepairPolicy && inboundDebugObserver
        ? createSseInspector({ onParsedPayload: payload => inboundDebugObserver.notePayload(payload) })
        : undefined;
      nativeExchange.inboundDebugUsesRawTerminalRepairTap = rawInboundSseInspector !== undefined
      ...
      if (forceCodexWsEagerRelay || eagerPath?.useEagerRelay || win32EagerRewrite || phaseBarrierRequiresEagerRelay) {
        ...
        const inspector = createSseInspector({
          ...
          logCtx,
          onCompletedResponse: rememberPassthroughResponse && responseEffects.plaintextV2AgentMessageToolNames.size === 0 ? rememberClientVisiblePassthroughResponse : undefined,
      ...
      if (recordTerminalOutcomes) {
        ...
        consumeForInspection(
          ...
          rememberPassthroughResponse && responseEffects.plaintextV2AgentMessageToolNames.size === 0 ? rememberClientVisiblePassthroughResponse : undefined,
