  ...
  if ("passthrough" in adapter && adapter.passthrough && !routedCompaction) {
    ...
    const rememberPassthroughResponseChecked = (
      ...
      if (nestedExecRepairCoordinator) {
        nestedExecRepairCoordinator.stageCacheCandidate(replayResponse, publishAcceptedResponse);
      } else {
    ...
    if (headers.get("content-type")?.toLowerCase().includes("application/json")) {
      ...
      try {
        rememberPassthroughResponseChecked(
          JSON.parse(nestedUpstreamJson) as { id?: unknown; output?: unknown; status?: unknown; model?: unknown },
        );
