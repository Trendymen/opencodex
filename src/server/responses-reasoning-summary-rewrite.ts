...
/** One retained sequence identity uses a conservative fixed metadata slot. */
...
export function createReasoningSummaryChannelBlockRewrite(options?: {
  ...
  let ownedTranslatorBudget: TranslatorBudget | undefined;
  ...
  const releaseSequenceIdentities = (state: PendingReasoning): void => {
    if (state.sequenceIdentityBytes > 0) {
      (options?.translatorBudget ?? ownedTranslatorBudget)?.releaseRetained(
        state.sequenceIdentityBytes,
        { kind: "item_ids" },
      );
  ...
  const disposeOwnedTranslatorBudget = (): void => {
    ownedTranslatorBudget?.dispose();
    ownedTranslatorBudget = undefined;
  };
  ...
  const rememberSequenceNumber = (state: PendingReasoning, sequenceNumber: number): boolean => {
    ...
    const translatorBudget = options?.translatorBudget ?? (ownedTranslatorBudget ??= createTranslatorBudget());
    translatorBudget.chargeRetained(SEEN_SEQUENCE_NUMBER_RETAINED_BYTES, { kind: "item_ids" });
  ...
  const rewrite: SseBlockRewrite = (block) => {
    ...
    if (payload.type === "response.output_item.done") {
      ...
      if (state && stateId !== null) {
        ...
        if (pending.size === 0) disposeOwnedTranslatorBudget();
        closed.set(stateId, {
          parts: [...state.parts],
        });
