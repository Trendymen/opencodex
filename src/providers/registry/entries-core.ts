...
export const PROVIDER_REGISTRY_CORE: readonly ProviderRegistryEntry[] = [
  ...
  */
  ...
  },
  {
    ...
    // Console Go 的 deepseek-flash 续轮曾因缺少 reasoning_text 返回 400。
    // 保留正文是机制推断，未做清空/保留对照；同 Provider 其他 Responses 模型尚未实测。
  ...
  { id: "lm-studio", label: "LM Studio (local)", adapter: "openai-chat", baseUrl: "http://localhost:1234/v1", authKind: "local", allowPrivateNetworkByDefault: true, allowBaseUrlOverride: true, featured: true, note: "Local — no key needed" },
  {
    ...
    modelInputModalities: {
      ...
      // 2026-09-10 的 Chat/Responses 纯色图实测确认此 legacy ID 可直接读取像素。
    ...
    */
    ...
    // Keep the official image-capable IDs direct; unprobed thinking IDs use the sidecar.
    noVisionModels: [
      "deepseek-chat",
      "deepseek-reasoner",
