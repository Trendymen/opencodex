...
已退休的 Spark ID 沒有模型專屬的 Responses Lite override。呼叫端或設定提供的 Lite 意圖會沿
canonical forwarding 的一般路徑處理。Lite 識別值改變時，舊 socket 會停止使用；後續識別值
相同且符合重用條件的請求可以重用新 socket。原生中繼資料格式不合法時，仍會退回 HTTP，並保持
請求本文不變。
