...
Kullanımdan kaldırılmış Spark kimliklerinde modele özgü bir Responses Lite override'ı yoktur.
Çağıranın veya yapılandırmanın verdiği Lite niyeti, canonical forwarding'in normal yolundan
işlenir. Lite kimliği değişince eski soket kullanım dışı bırakılır; aynı kimliğe sahip sonraki
uygun istekler yeni soketi yeniden kullanabilir. Bozuk yerel meta verilerde, istek gövdesi
