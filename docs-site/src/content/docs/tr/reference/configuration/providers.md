...
`quota`, kullanım eşiği aşıldıktan sonraki isteğinde onu yeniden bağlayabilir;
`pool.cacheAffinity` açıkken bu yeniden bağlama, bağlı hesap tükenene veya
hizmet veremez hale gelene kadar bekler. Duraklatma, soğuma, yeniden kimlik
doğrulama ve arıza işleme ise yönlendirmeyi bağımsız olarak temizleyebilir veya
taşıyabilir. Bağımsız bir
