...
| `models?` | `string[]` | 初始/回退模型列表。`liveModels: false` 时，非空 `models` 后接 `retainModels`；若 `models` 为空或省略，则按已配置的 `defaultModel`、`retainModels` 顺序构建初始列表，重复 ID 仅保留首次出现的位置。 |
...
| `defaultMaxOutputTokens?` | `number` | 当客户端省略 `max_output_tokens` 时的提供者级回退值；适用于 `openai-chat`，以及使用 key 鉴权的 `openai-responses` 请求。 |
| `modelMaxOutputTokens?` | `Record<string, number>` | 正数型、按模型设置的回退预算；精确或模式匹配优先于提供者默认值，生效线路相同。 |
...
| `inferResponsesMessagePhaseModels?` | `string[]` | 原生 `openai-responses` 上游缺少 assistant message `phase` 的精确模型 ID，默认关闭。仅对缺少 phase 的文本推断：后续仍有工作时标记为 `commentary`，正常完成的终态文本标记为 `final_answer`；不会生成、复制或摘要任何文字。上游已有 phase 保持不变，只有一处例外：`output_item.added` 宣布为 `final_answer` 时，仍无法判断后面是否还有工作，因此这次宣布在到达客户端前先降为 `commentary`，该 item 的最终相位由它自己的 `done` 与终态 snapshot 给出。若同一 item 的最终 snapshot 与已经观察到的后续工作相矛盾，则会将这个异常 snapshot 归一为已被证明的 `commentary`。失败/不完整回合绝不会合成 `final_answer`，但已经由后续工作证明的真实中间文本会保留为 `commentary`；即使列入配置，模型 ID 含 `gpt` 或 `openai` 时也始终排除。 |
...
`liveModels: false` 时，若 `models` 为空或省略，初始列表先加入已配置的 `defaultModel`，
...
实时发现会在缓存前拒绝超过 4 MiB 或 2,000 条原始模型行；内置预设可能使用更低的限制，并过滤为可聊天的行。过大或格式错误的结果会走陈旧/配置回退。合法的、零可用结果的发现仍然具有权威性，不会被静默替换或截断。
