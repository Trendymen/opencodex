...
은퇴한 Spark ID에는 모델별 Responses Lite override가 없습니다. 호출자 또는 구성에서 지정한
Lite 의도는 canonical forwarding의 일반 경로로 처리됩니다. Lite 식별값이 바뀌면 기존 소켓은
사용을 종료하며, 이후 같은 식별값으로 재사용 조건을 충족하는 요청은 새 소켓을 재사용할 수 있습니다.
네이티브 메타데이터 형식이 잘못된 경우에는 본문을 바꾸지 않고 HTTP로 폴백합니다.
