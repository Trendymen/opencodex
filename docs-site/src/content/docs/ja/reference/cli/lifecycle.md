...
すべての復元手順が成功した場合にのみ、サービスとプロキシを停止し、サービスと Codex シムを削除し、ネイティブ Codex を復元してから、opencodex ローカル設定を削除します。 `remove` は `uninstall` の別名です。設定のクリーンアップには所有権メタデータが必要です。新規インストールは空のディレクトリに書き込み、`runtime-port.json` のような opencodex の実行時状態ファイルがあるディレクトリは次回の書き込み時に引き継ぎます。クリーンアップが削除するのは opencodex が登録したパスと、ディレクトリごとのカタログバックアップだけです。それ以外はそのまま残り、残存として報告されます。
