...
退役した Spark ID にはモデル固有の Responses Lite override はありません。呼び出し元または
設定で指定された Lite 意図は canonical forwarding の通常経路で処理されます。Lite 識別値が
変わると古いソケットは退役し、以後の条件を満たす同じ識別値のリクエストは新しいソケットを
再利用できます。不正なネイティブメタデータは本文を変更せずに HTTP にフォールバックします。
