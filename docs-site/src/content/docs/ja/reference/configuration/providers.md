...
| `defaultMaxOutputTokens?` | `number` | クライアントが `max_output_tokens` を省略した場合のプロバイダー全体のフォールバック。`openai-chat` と、キー認証の `openai-responses` リクエストに適用されます。 |
| `modelMaxOutputTokens?` | `Record<string, number>` | モデルごとの正のフォールバック予算。完全一致やパターン一致はプロバイダーのデフォルトより優先され、適用される経路も同じです。 |
