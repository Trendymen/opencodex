...
Les identifiants Spark retirés n’ont pas de dérogation Responses Lite propre au modèle. L’intention
Lite fournie par l’appelant ou la configuration suit le chemin ordinaire de transmission canonique.
Un changement d’identité Lite retire l’ancien socket ; les requêtes admissibles suivantes ayant la
même identité peuvent réutiliser le nouveau socket. Des métadonnées natives mal formées entraînent
toujours un repli HTTP, sans modifier le corps.
