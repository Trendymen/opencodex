...
Восстановленный каталог исключает снятые нативные модели, включая `gpt-5.3-codex-spark`,
как с bare ID, так и с доверенным префиксом аккаунта. Это действует с резервной копией и без неё;
исходная копия и сохранённые пользователем прежние настройки выбора моделей остаются нетронутыми.
...
`remove` — alias команды `uninstall`. Для очистки конфигурации нужны ownership metadata: свежая
установка записывает их в пустой каталог, а каталог, где уже есть файл состояния opencodex вроде
`runtime-port.json`, получает их при следующей записи. Очистка удаляет зарегистрированные
opencodex пути и его собственные каталожные бэкапы; всё остальное остаётся на месте как residual.
...
`ocx service restart`. В macOS рассинхронизация версий оставляет определение службы байт-в-байт неизменным, поэтому `ocx service repair` ничего не перезагрузит и старый процесс продолжит обслуживание. Если новее прокси, обновите CLI
...
перезапустить подходящие процессы `codex … app-server` и `codex-code-mode-host` **и** полностью
закрыть и заново запустить Desktop-приложение Codex на macOS, Linux и Windows, чтобы model
picker перечитал каталог. Живые conversation'ы при этом завершаются. Широкий
`pkill -f codex` намеренно не используется.

`--restart-desktop-app` — устаревший alias `--restart-codex`. Он по-прежнему работает, печатает
предупреждение об устаревании и не ограничен Windows.

`--restart-app-server-only` возвращает прежнее узкое поведение: `SIGTERM` только подходящим
процессам app-server / code-mode-host текущего пользователя, Desktop-приложение остаётся
запущенным (активные turn'ы всё ещё могут оборваться). Если флаг указан вместе с
`--restart-codex` или `--restart-desktop-app`, побеждает узкий scope: потеря живых
conversation'ов невосстановима, а устаревший picker — нет.

Если команда запущена изнутри приложения Codex, restart передаётся detached helper'у, и эта
сессия завершается вместе с приложением.
...
opencodex. Предупреждение о stale-`app-server` и те же optional restart-флаги работают так же, как
и у `ocx sync`.
...
у `ocx sync` и `ocx sync-cache`, и применяются только после реальной записи. Условные запросы
`ETag` в эту команду не входят. Полная `--json`-обёртка и коды выхода описаны в
...
В Linux systemd-юнит вызывает первый обычный исполняемый файл `ocx`, найденный в `PATH` на момент
установки, а не пути Bun и CLI внутри установленного дерева пакета. Менеджеры версий вроде **mise** и
**asdf** устанавливают в каталог с версией и удаляют старый при обновлении; их стабильный shim
сохраняет разрешение юнита. Исходные checkout'ы без launcher'а `ocx` сохраняют прямую форму Bun + CLI.
Доверенный `OPENCODEX_BUN_PATH`, выбранный до запуска Bun, сохраняется через shim; пути к Bun,
поставляемому внутри пакета, перенаходятся после обновлений.

В macOS launchd, напротив, использует пути Bun и CLI внутри пакета, выбранные при install или repair.
Это не даёт изменяемому PATH-shim получить service API token и настроенное окружение прокси при
последующем restart. После обновления установки через менеджер версий выполните `ocx service repair`,
чтобы обновить эти пути перед перезапуском службы.

Определения, установленные до этого изменения, по-прежнему содержат старые версионные пути и не могут
мигрировать сами — после удаления старого исполняемого файла ни один код opencodex уже не запустится,
чтобы это исправить. Выполните `ocx service repair` один раз после обновления. После этого запуски
службы в Linux следуют за launcher'ом, а repair в macOS записывает новые пути пакета в определение
launchd. Уже работающий прокси внешним обновлением не заменяется: если установленный CLI новее
работающего прокси, выполните `ocx service restart`, чтобы обслуживалась новая сборка. В macOS здесь
`repair` недостаточно: определение не изменилось, а repair, который ничего не меняет, ничего не
перезагружает. Если новее прокси, проверьте установку CLI и `PATH`, как описано в
[`ocx status`](#ocx-status---json).

| Подкоманда | Действие |
| --- | --- |
| none | Установить и запустить службу, если её нет; иначе выполнить `repair` для существующей службы. Исправная конфигурация Windows Task Scheduler используется повторно; устаревшая может быть перерегистрирована и потребовать повышения прав. |
| `install` | Создать и запустить службу. |
| `repair` | Обновить установленную службу на месте. В macOS менеджер перезагружается только при наличии изменений, поэтому исправная неизменённая задача продолжает работать и обновление не является простоем. В Linux и Windows служба перезапускается; исправная конфигурация Windows Task Scheduler используется повторно, а устаревшая может быть перерегистрирована и потребовать повышения прав. |
| `restart` | То же обновление с гарантированным перезапуском на всех платформах. В macOS неизменённая, уже загруженная задача перезапускается на месте (kickstart). Не псевдоним команды `repair`. |
...
После обновления OpenCodex пересоздайте существующую обёртку Windows командами `ocx codex-shim uninstall`, затем `ocx codex-shim install`, чтобы применить это поведение. Обычное обновление не перезаписывает исправную обёртку Windows.
:::
...
немедленно.
Устарело: приложение OpenCodex для рабочего стола предоставляет трей в Windows, macOS и Linux;
`ocx tray` остаётся для установок без приложения для рабочего стола.
Когда становится известна новая версия пакета, трей добавляет синюю точку к значку онлайн, предупреждения или офлайн и показывает **Update available**. Локально кэшированный badge проверяется примерно раз в минуту; устаревшие или недоступные результаты убирают точку. Пункт меню открывает панель управления, где можно запустить обновление пакета. Автоматическая установка не выполняется.

## Дашборд

### `ocx gui`

Открыть [веб-дашборд](/guides/web-dashboard/) по адресу `http://localhost:<port>` (или `http://127.0.0.1:<порт управления>`, если на хабе включён management ingress), автоматически
...
Если OpenCodex установлен через mise, команда завершается с ошибкой до остановки прокси или изменения файлов пакета и показывает `mise upgrade <tool>` с проверенным локальным псевдонимом mise. Проверка обновлений остаётся доступной и сообщает о внешнем управлении. Нечитаемые или противоречивые метаданные владельца mise также запрещают изменения без угадывания имени инструмента, а `--tag preview` никогда не меняет выбранную в mise версию.
