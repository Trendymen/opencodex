...
У снятых с поддержки Spark ID нет модели-специфичного переопределения Responses Lite. Намерение
Lite, заданное вызывающей стороной или конфигурацией, проходит обычный путь канонической
пересылки. Изменение идентичности Lite выводит старый сокет из использования; последующие
подходящие запросы с той же идентичностью могут повторно использовать новый сокет. Некорректные
нативные метаданные по-прежнему приводят к откату на HTTP без изменения тела запроса.
...
- `codex/catalog/sync.ts`, экспортируемый через фасад `codex/catalog.ts`, сливает маршрутизируемые
  ...
  [модели подагентов](/ru/guides/codex-integration/#picker-подагентов) первыми,
